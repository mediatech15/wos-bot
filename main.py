...
if sys.prefix == sys.base_prefix and not is_container():
    print(
        "Running the bot in a venv (virtual environment) to avoid dependency conflicts."
    )
    venv_path = "bot_venv"
    ...
    if not os.path.exists(venv_path):
        try:
            ...
            if sys.platform == "win32":
                ...
                print(
                    "To continue, please run the script again with the venv Python:"
                )
                print(
                    f"  1. Open CMD or PowerShell in this directory: {os.getcwd()}"
                )
                print(
                    f"  2. Run: {venv_python_name} {os.path.basename(sys.argv[0])}"
                )
                sys.exit(0)
            else:  # For non-Windows, try to relaunch automatically
                print("Restarting script in virtual environment...")
                venv_python_executable = os.path.join(venv_path, "bin",
                                                      "python")
                os.execv(venv_python_executable,
                         [venv_python_executable] + sys.argv)
            ...
            print(
                "See also: https://docs.python.org/3/library/venv.html#how-venvs-work"
            )
            sys.exit(1)
    else:  # Venv exists
        if sys.platform == "win32":
            ...
            print(
                "To ensure you are using it, please run the script with the venv Python:"
            )
            print(
                f"  1. Open CMD or PowerShell in this directory: {os.getcwd()}"
            )
            ...
            if os.path.exists(venv_python_executable):
                print(
                    f"Using existing virtual environment at {venv_path}. Restarting..."
                )
                os.execv(venv_python_executable,
                         [venv_python_executable] + sys.argv)
            else:
                ...
                print(
                    "Please remove it and run the script again, or create a new one manually."
                )
                sys.exit(1)
...
def ensure_requirements_file():
    ...
    try:
        ...
        if latest_release_resp.status_code == 200:
            ...
            download_url = latest_release_data["assets"][0][
...
def check_and_install_requirements():
    ...
    for requirement in requirements:
        ...
        if package_name.lower() in [
                "ddddocr", "opencv-python-headless", "numpy", "pillow",
                "onnxruntime"
        ]:
            continue
    ...
    if missing_packages:  # Install missing packages
        ...
        for package in missing_packages:
            try:
                ...
                if package.startswith("ddddocr") and sys.version_info >= (3,
                                                                          13):
...
def check_ocr_dependencies():
    ...
    try:
        ...
        if "DLL" in str(e):
            ...
            print(
                "Please install latest 64-bit from: https://aka.ms/vs/17/release/vc_redist.x64.exe"
            )
        return False
...
if not setup_dependencies():
    print(
        "Dependency setup failed. Please install manually with: pip install -r requirements.txt"
    )
    sys.exit(1)
...
try:
    ...
    original_create_default_https_context = getattr(
    ...
    if original_create_default_https_context is None or \
       original_create_default_https_context is ssl.create_default_context:
        ...
        print(
            Fore.GREEN +
            "Applied SSL context patch using certifi for default HTTPS connections."
            + Style.RESET_ALL)
    else:  # Assume if it's already patched, it's for a good reason, just log it.
        print(
            Fore.YELLOW +
            "SSL default HTTPS context seems to be already modified. Skipping certifi patch."
            + Style.RESET_ALL)
except ImportError:
    print(
        Fore.RED +
        "Certifi library not found. SSL certificate verification might fail until it's installed."
        + Style.RESET_ALL)
except Exception as e:
    print(Fore.RED + f"Error applying SSL context patch: {e}" +
          Style.RESET_ALL)
...
if __name__ == "__main__":
    ...
    def safe_remove_file(file_path):
        ...
        if os.path.exists(file_path) and os.path.isfile(file_path):
            try:
                ...
                print(
                    Fore.YELLOW +
                    f"Warning: Access Denied. Could not remove '{file_path}'. Check permissions or if file is in use."
                    + Style.RESET_ALL)
            except OSError as e:
                print(Fore.YELLOW +
    ...
    def install_packages(requirements_txt_path: str,
        ...
        for dependency in lines:
            ...
            if dependency.startswith("ddddocr") and (
    ...
    async def check_and_update_files():
        ...
        if latest_release_resp.status_code == 200:
            ...
            if current_version != latest_tag:
                print(Fore.YELLOW + f"New version available: {latest_tag}" +
                      Style.RESET_ALL)
                print("Update Notes:")
                ...
                if not is_container():
                    ...
                    print(Fore.YELLOW +
                          "Running in a container. Skipping update prompt." +
                          Style.RESET_ALL)
                    update = True

                if update:
                    if os.path.exists("db") and os.path.isdir("db"):
                        print(Fore.YELLOW + "Making backup of database..." +
                              Style.RESET_ALL)

                        db_bak_path = "db.bak"
                        if os.path.exists(db_bak_path) and os.path.isdir(
                                db_bak_path):
                            try:
                                shutil.rmtree(db_bak_path)
                            except (
                                    PermissionError, OSError
                            ) as e:  # Create a timestamped backup to avoid upgrading without first having a backup
                                db_bak_path = f"db.bak_{int(datetime.now().timestamp())}"
                                print(
                                    Fore.YELLOW +
                                    f"WARNING: Couldn't remove db.bak folder: {e}. Making backup with timestamp instead."
                                    + Style.RESET_ALL)

                        try:
                            shutil.copytree("db", db_bak_path)
                            print(Fore.GREEN +
                                  f"Backup completed: db → {db_bak_path}" +
                                  Style.RESET_ALL)
                        except Exception as e:
                            print(
                                Fore.RED +
                                f"WARNING: Failed to create database backup: {e}"
                                + Style.RESET_ALL)
                    ...
                    if download_resp.status_code == 200:
                        ...
                        if os.path.exists("update") and os.path.isdir(
                                "update"):
                            try:
                                ...
                                print(
                                    Fore.RED +
                                    f"WARNING: Could not remove previous update directory: {e}"
                                    + Style.RESET_ALL)
                                return

                        try:
                            shutil.unpack_archive("package.zip", "update",
                                                  "zip")
                        except Exception as e:
                            print(
                                Fore.RED +
                                f"ERROR: Failed to extract update package: {e}"
                                + Style.RESET_ALL)
                            return
                        ...
                        if os.path.exists("update/main.py"):
                            ...
                            try:
                                ...
                                print(Fore.YELLOW +
                                      f"Could not backup main.py: {e}" +
                                      Style.RESET_ALL)
                                try:  # If backup fails, just remove the current file
                                    if os.path.exists("main.py"):
                                        os.remove("main.py")
                                        print(Fore.YELLOW +
                                              "Removed current main.py" +
                                              Style.RESET_ALL)
                                except Exception as _:
                                    print(
                                        Fore.RED +
                                        "Warning: Could not backup or remove current main.py"
                                        + Style.RESET_ALL)
                            ...
                            try:
                                ...
                                print(
                                    Fore.RED +
                                    f"ERROR: Could not install new main.py: {e}"
                                    + Style.RESET_ALL)
                                return
                            ...
                            for file in files:
                                ...
                                if os.path.exists(dst_path):
                                    ...
                                    try:
                                        ...
                                        print(
                                            Fore.YELLOW +
                                            f"Could not create backup of {dst_path}: {e}"
                                            + Style.RESET_ALL)
                                ...
                                try:
                                    ...
                                    print(
                                        Fore.RED +
                                        f"Failed to copy {file} to {dst_path}: {e}"
                                        + Style.RESET_ALL)
                        ...
                        try:
                            ...
                            print(
                                Fore.RED +
                                f"WARNING: update folder could not be removed: {e}. You may want to remove it manually."
                                + Style.RESET_ALL)
                        ...
                        print(
                            Fore.GREEN +
                            "Update completed successfully. Restarting bot..."
                            + Style.RESET_ALL)
                        restart_bot()
                    else:
                        print(
                            Fore.RED +
                            "Failed to download the update. HTTP status: {download_resp.status_code}"
                            + Style.RESET_ALL)
                        return
        else:
            print(
                Fore.RED +
                f"Failed to fetch latest release info. HTTP status: {latest_release_resp.status_code}"
                + Style.RESET_ALL)
    ...
    connections = {
        name: sqlite3.connect(path)
        for name, path in databases.items()
    }

    print(Fore.GREEN +
          "Database connections have been successfully established." +
          Style.RESET_ALL)

    def create_tables():
        with connections["conn_changes"] as conn_changes:
            conn_changes.execute(
    ...
    async def load_cogs():
        cogs = [
            ...
            "bot_operations", "logsystem", "support_operations",
            "gift_operations", "changes", "w", "wel", "other_features",
            "bear_trap", "id_channel", "backup_operations", "bear_trap_editor"
        ]
    ...
    async def on_ready():
        try:
            print(
                f"{Fore.GREEN}Logged in as {Fore.CYAN}{bot.user}{Style.RESET_ALL}"
            )
            await bot.tree.sync()
        except Exception as e:
